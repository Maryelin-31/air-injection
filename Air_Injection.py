...
def workspace(dist, rate_inj):
    ...
    temp_inje = 900
    # for x in (12.5, 30):
    cell = model.get_nearest_cell(pos=(25, 25, 1))
    ...
    def save_wt(path):
        ...
        result_folder = os.path.join(os.getcwd(), f'data_{name}', f'Results_{dist}_{rate_inj}', 'wt_cells')
    ...
    def save_mass(path):
        name = os.path.basename(__file__)
        result_folder = os.path.join(os.getcwd(), f'data_{name}', f'Results_{dist}_{rate_inj}', 'Results_cells')
    ...
    def save_sat(path):
        name = os.path.basename(__file__)
        result_folder = os.path.join(os.getcwd(), f'data_{name}', f'Results_{dist}_{rate_inj}', 'Saturation_cells')
    ...
    def run():
        ...
        resu_path = os.path.join(data_path, f'Results_{dist}_{rate_inj}')
        os.makedirs(os.path.join(os.getcwd(), data_path, f'Results_{dist}_{rate_inj}'), exist_ok=True)
    
        # folder = os.getcwd()
        folder = os.path.join(os.getcwd(), data_path, f'Results_{dist}_{rate_inj}')
        ...
        for step in range(1000000):
            ...
            if step % 100 == 0:
                ...
                monitor.save(os.path.join(folder, f'production.txt'))
                print(f'{step} {time / (3600 * 24 * 365)}')
        
    run()    
    
rate = np.linspace(50, 150, 6)
distant = np.linspace(50, 100, 3)

for d in distant:
    for r in rate:
        workspace(dist=d, rate_inj=r)
